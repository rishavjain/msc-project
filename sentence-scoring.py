...
class CsEmbeddingInferrer(CsInferrer):
    ...
    # def get_deps(self, sent, target_ind, substitute):
    #
    #
    #     return ldeps

    @staticmethod
    def get_target_index(lst_instance, conll):
        ...
        if cur_target_ind == 0:
            sys.stderr.write("ERROR: Couldn't find a match for target. {}: {}\n".format(lst_instance.target_ind,
                                                                                        lst_instance.target))
            cur_target_ind = lst_instance.target_ind + 1

        return cur_target_ind

    def sentence_score(self, lst_instance, conll, candidate, target):
        target_ind = self.get_target_index(lst_instance, conll)

        deps = {}

        for word_line in conll[1:]:
            parent_line = conll[word_line.head]
            if word_line.deptype == 'prep':
                continue
            if word_line.deptype == 'pobj' and parent_line.id != 0:
                grandparent_line = conll[parent_line.head]
            ...
            if ih == target_ind:
                head = candidate
            elif word_line.id == target_ind:
                w = candidate

            deps[(word_line.id, w)] = (ih, head, relation)
        ...
        for m in deps:
            ...
            # relation between m and d1
            d1 = tuple(["I_".join((x[2], x[1])) for x in d])
            while len(d1) > 0:
                if '::|::'.join(d1) in self.context_vecs.wi and m[1] in self.word_vecs.wi:
                    # print(m[1], '::|::'.join(d1))
                    # ldeps.append('::|::'.join(d1))
                    wvec = self.word_vecs.represent(m[1])
                    cvec = self.context_vecs.represent('::|::'.join(d1))

                    if m[1] in score:
                        score[m[1]] = max(score[m[1]], np.dot(wvec, cvec))
                    else:
                        score[m[1]] = np.dot(wvec, cvec)
                d1 = d1[:-1]
            ...
            while len(d1) > 0:
                if ih == target_ind and '::|::'.join(d1) in self.context_vecs.wi and h in self.word_vecs.wi:
                    # ldeps.append('::|::'.join(d1))
                    wvec = self.word_vecs.represent(h)
                    cvec = self.context_vecs.represent('::|::'.join(d1))

                    if h in score:
                        score[h] = max(score[h], np.dot(wvec, cvec))
                    else:
                        score[h] = np.dot(wvec, cvec)
        ...
        tvec = self.word_vecs.represent(target)
        for m in deps:
            if m[1] in self.word_vecs.wi and m[1] not in score:
                wvec = self.word_vecs.represent(m[1])
                score[m[1]] = np.dot(tvec, wvec)

        return np.array(list(score.values()))

    def find_scores(self, lst_instance, tfo, conll, canditates, measure):
        ...
        for candidate in canditates:
            sentence_score = self.sentence_score(lst_instance, conll, candidate, target)

            svec = self.word_vecs.represent(candidate)
            tvec = self.word_vecs.represent(target)
            tscore = np.dot(tvec, svec)

            if measure == 's-add':
                tscore = tscore + sentence_score.sum()
            elif measure == 's-mult':
                tscore = (tscore + 1)/2
                sentence_score = (sentence_score + 1)/2
                tscore = tscore * sentence_score.prod()
            else:
                raise Exception('Unknown context math: %s' % measure)

            result_vec.append((candidate, tscore))

        if len(result_vec) > 0:
            tfo.write("Candidate score: " + vec_to_str(result_vec, self.top_inferences_to_analyze) + '\n')
        else:
            tfo.write("Candidate score: None\n")
    ...
    def get_candidates(self, target, target2candidates):
        if target in target2candidates:
            candList = list(target2candidates[target])
            ...
            for candidate in candList:
                ...
                if candidate in self.word_vecs.wi:
                    candSet.add(candidate)

                for wn_pos in [wordnet.NOUN, wordnet.ADJ, wordnet.VERB, wordnet.ADV]:
                    if '_'.join([candidate, wn_pos]) in self.lemmas:
                        lemma = self.lemmas['_'.join([candidate, wn_pos])]
                        if lemma in self.word_vecs.wi:
                            candSet.add(lemma)
...
def thread_run(file_objects, inferrer, target2candidates, context_lines, measures):
    for context_line, conll in context_lines:
        ...
        candidates = inferrer.get_candidates(lst_instance.target_key, target2candidates)

        for measure in measures:
            if candidates is not None:
                result_vec = inferrer.find_scores(lst_instance, tfo_s, conll, candidates, measure)
            else:
                result_vec = None
